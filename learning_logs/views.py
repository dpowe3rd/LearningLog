...
def check_topic_owner(owner, user):
    """Function to check the registered user versus the user that created the given topic."""
    if owner != user:
        raise Http404
    return
...
def new_entry(request, topic_id):
    ...
    if request.method != 'POST':  # No data submitted; Create a blank form.
        ...
        if form.is_valid():
            ...
            new_entry.save()
            return HttpResponseRedirect(reverse_lazy('learning_logs:topic', args=[topic_id]))
...
def edit_entry(request, entry_id):
    ...
    if request.method != 'POST':
        ...
        if form.is_valid():
            form.save()
            return HttpResponseRedirect(reverse_lazy('learning_logs:topic', args=[topic.id]))
